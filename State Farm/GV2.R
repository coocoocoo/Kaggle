...
#GV2 is the vector version of original GV
GV2 <- function(img){
  ...
  gv.x <- t.img[2:(dim(t.img)[1]-1),3:dim(t.img)[2]]-t.img[2:(dim(t.img)[1]-1),1:(dim(t.img)[2]-2)]
  gv.y <- t.img[3:dim(t.img)[1],2:(dim(t.img)[2]-1)]-t.img[1:(dim(t.img)[1]-2),2:(dim(t.img)[2]-1)]
  gv.angle <- rad2deg(atan((gv.x+0.00001)/(gv.y+0.00001)))
  gv.magnitude <- sqrt(gv.x^2 + gv.y^2)
  ########
  #for(i in 1:img.row){
  #  for(j in 1:img.col){
